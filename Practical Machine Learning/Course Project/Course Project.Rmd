...
training <- training[-c(1:6,60)]
cross_val <- cross_val[-c(1:6,60)]
```
...
```
We see that many of the columns are highly correlated with each other. Let's train a PCA model with 2 principle components:

```{r}
preProc <- preProcess(training,method="pca",pcaComp=2)
pred_training <- predict(preProc,training)
...
```{r}
# Add `classe` vector to the principle component predictors...
...
First, let's determine the principle component predictions for the cross-validation dataset based on our preprocessing parameters for the training dataset.

```{r}
pred_cross_val <- predict(preProc,cross_val)
...
Now, let's test the model with the test data. We need to recall what columns of the training dataset we kept so that the columns of the test dataset are identical.
